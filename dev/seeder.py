...
async def seed(session: SessionDep) -> dict[str, int]:
    ...
    if os.getenv('ENV') == Envs.development:
        ...
        dict_ = dict(
            accounts=account_count, roles=roles_count, options=sys_options_count, apikeys=apikeys_count,
            exchanges=exchanges_count, orders=order_count, trades=trade_count,
        )
...
#     start_time = arrow.get('2025-04').int_timestamp * 1000
#     await AppSeeder.seed_trades(start_time, session=session)
...
class AppSeeder:
    ...
    async def seed_orders(start_time: int, session: AsyncSession):
        ...
        next_pointer = arrow.utcnow().int_timestamp * 1000
        ...
        if session.new:
            await session.commit()
            await account.update_options({'pointer_all_orders': next_pointer}, session=session)
        return total
    ...
    async def seed_trades(start_time: int, session: AsyncSession):
        ...
        if session.new:
            await session.commit()
            await account.update_options({'pointer_all_trades': next_pointer}, session=session)
        return total
