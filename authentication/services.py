...
class AccountSvc:
    ...
    async def ban_user(*, authorization: 'Account', to_ban: 'Account', notes: str,
        ...
        """
        try:
            ban = auth_.Ban(account_id=to_ban.id, implementor_id=authorization.id, notes=notes)
            session.add(ban)
    ...
    async def unban_user(*, authorization: 'Account', to_ban: 'Account', session: AsyncSession) -> 'Account':  # noqa
        ...
        """
        try:
            stmt = (update(auth_.Ban)
                    .where(auth_.Ban.account_id == to_ban.id, auth_.Ban.implementor_id == authorization.id,  # noqa
                           auth_.Ban.is_active == True)  # noqa
